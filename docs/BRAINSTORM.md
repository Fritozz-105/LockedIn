...
| D85 | ~~**Expo SDK 54** (current-stable Jun 2026): React 19.1.0, React Native 0.81, Node 20.19.x~~ — **SUPERSEDED by D107** | ⤴️ | Was correct on 2026-06-08, but by 2026-06-29 SDK 54 was two majors behind current-stable. The *reasoning* ("build on current-stable, not bleeding edge") now points at SDK 56. Original note: picked current-stable over bleeding edge for fewer first-timer landmines. Clarifies D72. |
...
| D108 | **JWT verification via `jose` (`createRemoteJWKSet` + `jwtVerify`) + Supabase asymmetric (ES256) signing keys**, supersedes D64's `@fastify/jwt` + `jwks-rsa`. Issuer `${SUPABASE_URL}/auth/v1`, audience `authenticated`. Requires enabling asymmetric signing keys in the Supabase dashboard (default is HS256, where the JWKS endpoint is empty). | ✅ | Supabase now documents `jose`; it handles ES256 *and* RS256 (jwks-rsa is RSA-centric, and Supabase's new asymmetric keys default to ECC) and bakes in JWKS caching/cooldown. Verified live 2026-06-30: `/me` verifies a real token and reads the matching `auth.users` row; no-token → 401. Per provenance, D64's library detail re-decided with current facts. |
| D107 | **Expo SDK 56** (current-stable 2026-06-29), supersedes D85. Scaffold pinned by the SDK 56 template: **React 19.2.3, react-dom 19.2.3, React Native 0.85.3** (in workspace-root `pnpm.overrides`, D72). Mobile uses **TypeScript ~6.0.3** (backend stays on 5.7 for now). Node still 20.19.x — verified: install + headless `expo export` (Metro bundle, 1483 modules) + strict typecheck all clean. | ✅ | D85's "current-stable" reasoning re-applied with current facts: SDK 54 had fallen two majors behind, so building a 12–16wk project on it means shipping stale. Decision made with the owner on 2026-06-29 per the provenance rule. **Open item:** SDK 56 bundles react-native-web/react-dom — the D85 note about dropping `react-dom` no longer applies. Revisit the Node 20 pin if any SDK 56 native tooling demands newer. |
